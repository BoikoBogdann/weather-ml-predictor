...
if st.session_state.df_weather is not None:
    ...
    with view_tab_hist:
        if st.session_state.rf_metrics is not None:
            ...
            with g_col2:
                st.plotly_chart(px.bar(st.session_state.feature_df, x="Лінійна кореляція з таргетом (r)", y="Ознака (Метеопараметр)", orientation='h', title="Лінійна кореляція Пірсона (r) з таргатом", color="Лінійна кореляція з таргетом (r)", color_continuous_scale="RdBu", range_color=[-1,1]), use_container_width=True)
